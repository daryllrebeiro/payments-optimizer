...
if (typeof globalThis.indexedDB === 'undefined') {
  ...
  class IDBTransaction {
    ...
    constructor(private db: any, storeNames: string | string[], private mode: string) {
  ...
  class IDBObjectStore {
    ...
    constructor(private name: string, private db: any) {}
      ...
      if (range && range.lower && range.upper) {
        results = results.filter((item: any) => {
          ...
          if (Array.isArray(this.keyPath)) {
            ...
            const merchantId = item.data[merchantKey];
            const timestamp = item.data[timestampKey];
            
            const lowerMatch =
              merchantId === range.lower[0] && timestamp >= range.lower[1];
            const upperMatch =
              merchantId === range.upper[0] && timestamp <= range.upper[1];
            
            return lowerMatch && upperMatch;
          } else {
            // Single key
            const value = item.data[this.keyPath];
        ...
        results = results.filter((item: any) => {
          if (!item || !item.data) return false;
          return item.data[this.keyPath as string] === range;
        });
      }
  ...
  class IDBOpenDBRequest extends IDBRequest {
    ...
    _triggerUpgrade(db: IDBDatabase, oldVersion: number, newVersion: number) {
      this.result = db;
    ...
    open(name: string, version: number = 1) {
      ...
      setTimeout(() => {
        ...
        if (!db || oldVersion < version) {
          ...
          request._triggerUpgrade(db, oldVersion, version);
        }
