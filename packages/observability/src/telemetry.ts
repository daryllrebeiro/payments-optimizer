...
import type { Clock } from '@payments-optimizer/resilience';
import { getClock } from '@payments-optimizer/resilience';
...
export const TelemetryEventType = {
  ...
  // Business events
  BENEFIT_APPLYED: 'benefit_applied',
...
export class Telemetry {
  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      getLogger().info(`Telemetry: Sent ${eventsToSend.length} events`);
    } catch (error) {
      // Re-queue failed events
      this.queue = [...eventsToSend, ...this.queue];
      getLogger().error('Telemetry: Failed to send events', { error: String(error) });
    }
  }
  ...
  private startFlushTimer(): void {
    setInterval(() => {
      this.flush().catch((error) => {
        getLogger().error('Telemetry: Flush failed', { error: String(error) });
      });
  ...
  private async sendEvents(events: TelemetryEvent[]): Promise<void> {
    if (this.config.endpoint && this.config.apiKey) {
      ...
      for (const event of events) {
        getLogger().debug(`Telemetry event: ${event.type}`, event.properties);
      }
    }
  }
  ...
  private queueEvent(event: TelemetryEvent): void {
    if (this.queue.length >= (this.config.maxQueueSize ?? 1000)) {
      ...
      this.queue.shift();
      getLogger().warn('Telemetry: Queue full, dropped oldest event');
    }
