...
async function main() {
  ...
  const twentyVoucherBench = results.find(r => 
    r.name.includes('20 vouchers') && r.name.includes('width=5') && !r.name.includes('Narrow') && !r.name.includes('Wide')
  );
  ...
  if (twentyVoucherBench) {
    ...
    if (twentyVoucherBench.maxTimeMs < p95Target) {
      console.log(`  ✓ PASS - Even worst case (${twentyVoucherBench.maxTimeMs.toFixed(2)}ms) is under P95 target`);
